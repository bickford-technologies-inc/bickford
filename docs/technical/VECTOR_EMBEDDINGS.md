...
Bickford architecture fit
-------------------------

This guide can support Bickford in three concrete layers: capture, index, and retrieve. The intent is to turn operational text into a durable, searchable memory that complements the Bickford Canon decision layer. Use embeddings only for similarity, grouping, or retrieval; keep the Canon as the deterministic truth layer. See the system context in the technical architecture documentation for how events flow into the ledger and Canon.【F:docs/technical/ARCHITECTURE.md†L1-L134】

### 1) Capture: what to embed

Embed human-readable summaries of key system events to create a semantic footprint you can search later. Typical candidates:

* Session completion summaries
* Tool execution summaries and errors
* Policy decisions with short rationales
* External signals (e.g., alerts, incidents, or advisory notes)

### 2) Index: where embeddings live

Store the embedding vectors alongside their event metadata in whatever store is already used for Bickford event retention (ledger, database, or external vector store). The vector store enables fast similarity search, while the ledger remains the source of truth for events and Canon output.

### 3) Retrieve: how Bickford uses embeddings

Use embeddings as an optional retrieval layer to surface:

* **Similar prior sessions** (accelerate triage or support)
* **Nearest neighbor decisions** (support human review of Canon outputs)
* **Anomaly clusters** (detect outliers in workflow failures)
* **Trend analysis** (group related incidents or workflows over time)

Embedding search should be advisory. The Canon layer and policy rules should remain the authority for decisions, while embeddings provide fast context and evidence.

### Bickford runtime module

The runtime now includes a lightweight, in-memory embedding index you can wire into the session completion flow or other operational pipelines. It exposes:

* `createInMemoryEmbeddingIndex(provider)` to store embeddings for a tenant.
* `indexSummary({ id, tenantId, createdAt, summary, metadata })` to write entries.
* `querySimilar(tenantId, query, options)` to retrieve nearest neighbors with optional filters.

Use it as a local prototype or as the interface you swap out when integrating a production vector store.
