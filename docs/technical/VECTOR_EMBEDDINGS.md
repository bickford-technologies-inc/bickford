...
## How embeddings fit into Bickford

Embeddings translate unstructured text into vectors, which makes them a practical backbone for Bickford’s intent-first decision flow. In Bickford terms, embeddings are a way to turn free-form evidence (session transcripts, user intent, tool outputs) into searchable, comparable signals that can be routed into the Canon decision layer.

### Bickford-aligned use cases

- **Semantic recall for intent**: find similar prior sessions or intents to bootstrap context without loading entire histories.
- **Evidence clustering**: group sessions or artifacts by similarity to reveal recurring patterns for promotion or escalation.
- **Recommendation support**: rank candidate next actions based on embedding similarity to past successful outcomes.
- **Anomaly detection**: flag sessions that are far from historical norms to trigger review workflows.
- **Classification**: assign intent labels or risk tiers by comparing embeddings to canonical label vectors.

### Bickford integration map (recommended)

1. **Define the decision objective**  
   What Bickford decision does this enable (search, routing, escalation, promotion evidence)?
2. **Select source fields**  
   Pick the smallest set of text that captures intent (session summary, final user request, tool output).
3. **Embed and store**  
   Generate embeddings once, store them in a vector index keyed by session or artifact ID.
4. **Retrieve and score**  
   Run cosine similarity against the index to pull the most relevant matches.
5. **Promote evidence**  
   Treat retrieved items as candidate evidence and gate them through Canon promotion rules before they influence decisions.
...
- **Compounding knowledge**: persist retrieval snapshots (query + retrieved matches) as ledger evidence so future decisions can reuse what worked and reduce repeated lookup costs.
- **Compounding dynamic performance**: track retrieval metrics (average/top similarity, match counts) to learn which prompts and sources improve recall over time.
...
def rank_similar(query: str, corpus: List[Dict[str, str]]) -> List[Dict[str, str]]:
    embeddings = embed_texts([query] + [item["text"] for item in corpus])
    query_embedding = embeddings[0]
    corpus_embeddings = embeddings[1:]
    scored = []
    for item, embedding in zip(corpus, corpus_embeddings):
...
corpus = [
    {"id": "sess_1", "text": "User wants to automate invoice reconciliation."},
    {"id": "sess_2", "text": "User asks for compliance risk guidance on contracts."},
]

results = rank_similar("Automate invoice matching for finance team", corpus)
print(results[:1])
```
