...
## Bickford usage guidance

Use TTS in Bickford when you want to turn the system's text outputs into a spoken interface for demos, executive briefings, or accessibility-first experiences. The key architectural decisions are about where to generate audio, how to store it, and how to preserve disclosure.

**Where to generate speech**

* **Server-side rendering** for predictable latency and consistent audio quality (best for demos, narrated reports, and pre-rendered artifacts).
* **Realtime streaming** for interactive experiences (best for live agents and conversational UI).

**How to wire it in**

1. **Identify the text source**: choose the canonical response you want spoken (e.g., the final decision summary or a synthesized narrative).
2. **Select the voice and format**: pick `marin` or `cedar` for quality, and prefer `wav`/`pcm` when low latency is critical.
3. **Generate audio**: call the `speech` endpoint with clear `instructions` so the tone matches the delivery context.
4. **Store and reference**: persist the audio file alongside the originating session so it is replayable and auditable.
5. **Disclose**: surface the required disclosure in the UI wherever playback is possible.

**Operational checklist**

* Cache deterministic outputs (e.g., reports) so reruns do not regenerate audio unnecessarily.
* Store metadata (voice, model, language, instructions) to make outputs reproducible.
* Use streaming only when the end-user experience benefits from real-time playback.

**Bickford API endpoint**

When running the web app, you can call the built-in TTS endpoint to generate audio without wiring OpenAI directly from the client.
...
{
  "input": "Bickford is ready to narrate this summary.",
  "voice": "marin",
  "voiceId": "voice_123abc",
  "format": "wav",
  "instructions": "Speak with calm, confident pacing.",
  "language": "en"
}
```

The endpoint requires `OPENAI_API_KEY` and returns the audio payload directly with an `audio/*` content type. Use `voiceId` for custom voices (it overrides `voice`) and `language` to guide pronunciation. Ensure your UI still discloses the voice is AI-generated.
