...
### Compounding value measurements

Hook data often feeds ROI tracking that compounds over time (for example, improvements per agent session or per workflow stage). Record both the instantaneous value and the compounding schedule so reports can be reconciled consistently across teams.

**Discrete compounding (example):**

```text
value_t = value_0 * (1 + r / n)^(n * t)
```

**Continuous compounding (example):**

```text
value_t = value_0 * e^(r * t)
```

**Example hook output payload for compounding analytics (JSON):**

```json
{
  "timestamp": 1704614700000,
  "metric": "time_saved_usd_per_hour",
  "value": 425.0,
  "currency": "USD",
  "compounding": {
    "mode": "continuous",
...
Tie hooks to measurable workflows so that operational value can be reported consistently. A single session might span multiple workflows; log an explicit `workflowId` and `stage` to correlate tool usage and outcomes.

**Example workflows:**

* Incident response: triage → reproduce → fix → validate → postmortem.
* Customer support: intake → diagnose → resolve → follow-up.
* Sales enablement: research → pitch prep → pricing review → contract handoff.
* Compliance review: evidence gather → policy check → audit log → sign-off.
* Infrastructure operations: capacity review → change plan → rollout → verification.

**Example hook log entry (JSON):**

```json
{
  "timestamp": 1704614700000,
  "workflowId": "incident-2024-01-08-2431",
  "workflowStage": "validate",
  "toolName": "bash",
  "toolArgs": "{\"command\":\"npm test\"}",
  "valueUsdPerHour": 310.0
}
```

### Measurable value groupings (USD per hour)

When reporting value, normalize to **USD per hour** so comparisons stay consistent across regions and business units. Capture dimensions at the time of tool use to preserve historical correctness.

**Common dimensions to record (representative list):**

* Region: global, north-america, emea, apac, latam, country, state, metro.
* Business unit: platform, infrastructure, security, data, product, marketing, sales, finance, legal, operations, customer-success.
* Sales region: na-east, na-west, emea-north, emea-south, apac-north, apac-south.
* KPIs: MTTR, change failure rate, deployment frequency, lead time, CSAT, NPS, churn, conversion rate.
* Cost center: engineering, cloud, go-to-market, compliance, support.
* Org: division, team, pod, squad, project, program.
* Customer segment: enterprise, mid-market, smb, public sector.
* Channel: direct, partner, marketplace.
* Role: SRE, security analyst, developer, designer, PM, AE, CSM.
* Tooling: IDE, CLI, CI/CD, ticketing, CRM, analytics.
* Time: fiscal quarter, fiscal year, sprint, on-call rotation.
* Location: office, remote, hybrid.
* Platform: AWS, Azure, GCP, on-prem, hybrid.
* Product line: core API, enterprise platform, developer tools, marketplace.
* Service tier: standard, premium, mission-critical.
* Risk class: low, medium, high.
* Regulatory scope: SOC2, ISO27001, HIPAA, GDPR, PCI.
* Revenue tie: ARR band, contract size, pipeline stage.

**Example value payload with dimensions:**

```json
{
  "timestamp": 1704614700000,
  "metric": "value_usd_per_hour",
  "value": 580.0,
  "currency": "USD",
  "dimensions": {
    "region": "emea",
