...
- **[Organization API Keys API](/docs/en/api/organizations-api-keys-update)**: Update an organization API key (`POST /v1/organizations/api_keys/{api_key_id}`)
...
Use the Organization API Keys API to update API key metadata or status.
...
- `status` (string, optional): One of `"active"`, `"inactive"`, or `"archived"`.

**Returns:** `APIKey` object including `id`, `created_at`, `created_by`, `name`, `partial_key_hint`, `status`, `type`, and `workspace_id`.
...
- [Get started](/docs/en/get-started)
- [Working with Messages](/docs/en/build-with-claude/working-with-messages)
- [Messages API Reference](/docs/en/api/messages)
- [Client SDKs](/docs/en/api/client-sdks)
