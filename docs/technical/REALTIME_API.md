...
## How this fits into Bickford

Use the Realtime API as a low-latency, voice-first interface that feeds Bickford’s execution authority layer. This keeps Bickford’s core flow (session completion → OPTR → canon → ledger) intact while adding a realtime input/output channel for voice or multimodal experiences. For the broader system flow, see [Bickford Architecture](../ARCHITECTURE.md).

### Intended use in the Bickford system flow

1. **Capture user intent via Realtime** (voice or multimodal).
2. **Normalize and log the session** with Session Completion Runtime (latency and usage data).
3. **Route intent to OPTR** for decision optimization and non-interference checks.
4. **Apply canon enforcement** before execution.
5. **Append outcomes to the ledger** for immutable auditability.

### Connection choices mapped to Bickford surfaces

- **WebRTC**: browser-based voice UI for Bickford’s front-end and demos.
- **WebSocket**: server-side proxy for controlled orchestration, logging, and guardrails.
- **SIP**: telephony entrypoint (call center or IVR) feeding the same Bickford pipeline.

### Why this matters for Bickford

- **Persistence (knowledge):** sessions are captured and anchored in the ledger.
...
- [WebRTC connection](https://platform.openai.com/docs/guides/realtime-webrtc)
  - Ideal for browser and client-side interactions with a Realtime model.
- [WebSocket connection](https://platform.openai.com/docs/guides/realtime-websocket)
  - Ideal for middle tier server-side applications with consistent low-latency network connections.
- [SIP connection](https://platform.openai.com/docs/guides/realtime-sip)
...
- **[Managing costs](https://platform.openai.com/docs/guides/realtime-costs):** Learn how to monitor and optimize your usage of the Realtime API.
...
Configuration for input modalities and other properties have moved as well, notably output audio configuration like model voice. [Check the API reference](https://platform.openai.com/docs/api-reference/realtime_client_events) for the latest event shapes.
