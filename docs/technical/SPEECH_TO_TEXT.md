...
Use speech-to-text when you want audio evidence to become searchable, reviewable text inside Bickford. The intent is to convert raw audio into structured artifacts that can be referenced in decisions, summaries, and ledgered outputs.

### What this enables

- **Persistence**: audio gets converted into text that can be stored, quoted, and versioned.
- **Automation**: transcript metadata (timestamps, speakers, language) makes it easier to index, tag, and route follow-up actions.
- **Direction**: clear transcripts make intent extraction and review more reliable than raw audio alone.

### Recommended flow

1. **Choose the model** based on your need:
   - Use `gpt-4o-mini-transcribe` for fast drafts.
   - Use `gpt-4o-transcribe` for higher quality transcripts.
   - Use `gpt-4o-transcribe-diarize` when speaker separation matters.
2. **Select the output shape**:
   - `text` for simple reading and quoting.
   - `json` or `diarized_json` when you need timestamps or speakers for downstream automation.
3. **Capture metadata** alongside the transcript:
   - Source file name, language, model, response format, and time range.
4. **Attach the transcript to your Bickford artifact** (intent, decision, or evidence record) so future steps can reference it directly.

### When to use diarization

Use diarization when:

- There are multiple speakers and attribution matters.
- You want to align decisions or action items to specific speakers.
- You plan to summarize or score each speaker separately.

### Bickford API endpoint

Bickford exposes a server-side transcription endpoint that proxies to OpenAI and records a trace entry for persistence.
...
```bash
curl --request POST \
  --url http://localhost:3000/api/audio/transcribe \
