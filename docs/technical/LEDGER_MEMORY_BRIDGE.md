...
See `packages/ledger/examples/memory-ledger.ts` for a runnable Bun + SQLite example
that wires hash-chained storage, embeddings, similarity search, and analytics into a
single ledger-backed component.
...
## Similarity, Nodes, Ada (Embedding Notes)

- **Cosine similarity** is the default scoring metric for vector retrieval. Normalize vectors and
  keep the scoring pipeline deterministic to avoid drift in rankings.
- **Nodes** refer to the normalized ledger documents that become retrieval units. Each node maps
  1:1 to a ledger entry and carries immutable metadata (`ledger_id`, `workspace_id`, `canon_id`).
- **Ada embeddings** (or any fixed embedding model) should be version-pinned. Store the embedding
  model name/version alongside each node so re-indexing is reproducible.
