...
### Value and compounding considerations

As you refine your plan, define how the business value compounds over time and how you measure it. This helps you prioritize features and create dashboards that reflect real-world outcomes.

- **Compound value tracking:** Track recurring value created by time saved or outcomes delivered. For example, if a team saves 10 hours per week at $120/hour, the weekly value is $1,200, and the monthly value compounds as tasks and automation coverage grow.
- **Continuous compounding:** For continuous processes (support queues, production systems, revenue workflows), calculate value on a rolling basis (daily or weekly), and project forward with a compounding rate that reflects improvements in throughput, quality, or cycle time.
- **Real use cases (business process workflows):**
  - **Sales enablement:** Time saved per proposal, multiplied by close rate improvements.
  - **Support operations:** Minutes saved per ticket, multiplied by ticket volume and reduced backlog.
  - **Finance operations:** Faster close time per month-end cycle.
  - **Engineering delivery:** Reduced lead time per feature, or reduced incident recovery time.

#### USD/hour value definition

Decide the base rate you will use to translate time into value. Typical approaches include:

- **Regional rate:** A standard $/hour for a region (e.g., North America, EMEA, APAC).
- **Business unit rate:** Differentiated rates for Sales, Marketing, Engineering, Finance, Support, etc.
- **Sales region rate:** Distinct $/hour by sales territory or market segment.
- **Role-based rate:** Rate by role or job family (e.g., AE, SE, SRE, PM).
- **KPI-based rate:** Rate tied to a KPI improvement (e.g., $/hour of cycle time reduction).

#### Enterprise grouping dimensions (example catalog)

Use a comprehensive grouping system so that value can be aggregated across the organization. Example groupings include:

- **Company:** Parent entity, subsidiary, region, legal entity.
- **Organization:** Business unit, department, team, cost center.
- **Geography:** Region, country, sales territory, timezone.
- **Role:** Job family, level, function, employee ID.
- **Process:** Workflow type, stage, system of record.
- **Customer:** Segment, industry, account tier, ARR band.
- **Product:** Product line, SKU, platform, feature set.
- **Revenue:** Deal stage, pipeline stage, ACV/ARR impact.
- **Risk:** Compliance tier, security classification, audit scope.
- **Time:** Daily, weekly, monthly, quarterly time buckets.
...
## Building your application with Copilot coding agent
