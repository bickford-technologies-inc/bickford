...
function main() {
  console.log(
    "[check-no-deep-imports] Scanning for deep cross-package imports...\n"
  );
  ...
  for (const dir of searchDirs) {
    ...
    try {
      ...
      if (err.status && err.status > 1) {
        console.error(
          `[check-no-deep-imports] Error scanning ${dir}:`,
          err.message
        );
        process.exit(1);
      }
  ...
  if (violations.length > 0) {
    console.error(
      "[check-no-deep-imports] ❌ FAILED: Found deep cross-package imports:\n"
    );
    ...
    console.error(
      '[check-no-deep-imports] Use package root imports only (e.g., "@bickford/core").'
    );
    console.error(
      "[check-no-deep-imports] Ensure all public APIs are exported in src/index.ts.\n"
    );
    process.exit(1);
  }
