...
const policy = JSON.parse(
  fs.readFileSync("infra/authority/execution-boundaries.json", "utf8")
);
...
for (const file of files) {
  ...
  const callsAuthorize = src.includes(`${policy.requiredSymbol}(`);

  if (!hasAuthorityImport || !callsAuthorize) {
    console.error(`❌ AUTHORITY VIOLATION: ${file}`);
    violations++;
  }
}
