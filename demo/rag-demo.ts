...
// Initialize
const memoryLedger = new MemoryLedger(demoDbPath);
...
async function demo() {
  ...
  console.log(
    "Expected: 2+ RAG matches (previous Constitutional AI queries)\n",
  );
  ...
  try {
    const result3 = await client.complete({
      messages: [
        {
          role: "user",
          content: "What are enterprise use cases for Constitutional AI?",
        },
      ],
      maxTokens: 500,
    });
  ...
  analytics.topicCoverage.forEach((topic) => {
    console.log(
      `  ${topic.topic}: ${topic.count} interactions (quality: ${topic.avgQuality.toFixed(2)})`,
    );
  });
  console.log("");
  ...
  console.log(
    `  Compliance Proof: ${integrity.valid ? "Cryptographically verified" : "Tampered"}`,
  );
  console.log("");
  ...
  console.log(
    "✓ Compliance: Hash-chained ledger provides tamper-evident audit trail",
  );
  console.log(
