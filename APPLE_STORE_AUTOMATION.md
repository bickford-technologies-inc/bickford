...
## Post-Rejection Checklist

If your iOS app is rejected by App Store Review, follow these steps:

### 1. Check Common Issues

- ✅ App icon is 1024×1024 PNG with no transparency
- ✅ `app.json` has `"ios.supportsTablet": false` (if iPhone-only)
...
```bash
cd packages/bickford-mobile-expo
bash scripts/validate-ios-prebuild.sh
```

### 3. Rebuild & Resubmit

```bash
npm run eas:build:ios      # Builds new binary
npm run eas:submit:ios     # Submits to App Store Connect
```
...
- Go to "Activity" tab
- Find the rejected build
- Click "Reply" 
- Message: "Issue resolved in build [new build number]. Ready for re-review."

### 5. Monitor Re-Review

- Apple typically re-reviews within 24-48 hours
- You'll receive an email when the new build is approved or if further issues are found
