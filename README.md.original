...
### Option 1: Template Button (Recommended)

[![Deploy on Railway](https://railway.app/button.svg)](https://railway.app/template/bickford?referralCode=bickford)
...
3. Links repository
4. Adds PostgreSQL
5. Deploys and generates domain

### Option 3: Manual Setup

If automation fails, follow [Manual Railway Setup Guide](./docs/railway-manual-setup.md).

---

## 🎯 Live Demo

**Bickford Execution Runtime**: https://session-completion-runtime.vercel.app

---

## 🏗️ Prime Architecture Initiative

This repo is being architected as the "prime infinite Bickford" – a single source of truth consolidating all Bickford repositories.

**Status**: 🟢 Milestone 1 - Discovery & Inventory  
**Track progress**: See [docs/ARCHITECTURE_PRIME.md](./docs/ARCHITECTURE_PRIME.md)  
**Trigger next step**: Run the "Architect Prime" workflow in Actions tab

**What this enables**:
- Single source of truth for all Bickford code
- Preserved git history across all projects
- Unified dependency management and CI/CD
- Automated consolidation with coaching checkpoints
...
- ✅ Start both backend and frontend with auto-recovery
- ✅ Monitor health and auto-restart on failures
- ✅ Open browser automatically

### 🔧 Self-Healing Automation

The system includes complete automation for zero-manual-intervention operation:
...
- ✅ Service crashes → Auto-restarted (30s monitoring)
- ✅ Bad configuration → Auto-corrected
- ✅ Missing dependencies → Auto-installed
- ✅ Build cache issues → Auto-cleared
- ✅ Syntax errors → Prevented by pre-commit hooks
- ✅ Failed deployments → Auto-rolled back via GitHub Actions
...
Bickford uses OpenAI to classify user intents and make authorization decisions. You can configure how strictly these decisions are enforced:

### Authorization Modes

Set `AUTH_DECISION_MODE` in `packages/bickford/.env`:

#### **Permissive (Default)** ✅ Recommended for most users
```bash
AUTH_DECISION_MODE=permissive
```
- Always allows requests
- OpenAI provides helpful conversational summaries
- No blocking behavior
- Best for: Development, personal use, exploratory workflows

#### **Audit** 🔍 Recommended for testing auth rules
```bash
AUTH_DECISION_MODE=audit
```
- Always allows requests (never blocks)
- Logs would-be DENY decisions as warnings
- Helps tune authorization rules without disrupting users
- Best for: Staging environments, testing authorization logic

#### **Enforce** 🔒 Recommended for production with strict access control
```bash
AUTH_DECISION_MODE=enforce
```
- Enforces OpenAI's ALLOW/DENY decisions
- Blocks requests that would violate security policies
- Requires well-tuned system prompt
- Best for: Production environments with sensitive data

### Example Configuration

```bash
# packages/bickford/.env
AUTH_DECISION_MODE=permissive
OPENAI_API_KEY=sk-...
...
The system prompt in `packages/bickford/api/server.min.ts` defines what gets ALLOWED vs DENIED. You can customize it for your specific use case.
