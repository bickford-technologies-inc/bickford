...
## Filing Workspace (5173) knobs

- **Time travel**: The topbar slider replays state via `GET /api/filing/state?at=<ISO>`.
- **Promotion (role-gated)**: Set `VITE_BICKFORD_ROLE=promoter` in `packages/demo-dashboard/.env` to enable Promote buttons and `bickford promote ...`.
- **LLM router (optional)**: Set `OPENAI_API_KEY` + `OPENAI_MODEL` in the API environment to enable `bickford ask ...`.
- **Ledger anchors (optional signing)**: Set either `BICKFORD_ANCHOR_HMAC_KEY` (HMAC) or `BICKFORD_ANCHOR_PRIVATE_KEY_PEM` (RSA) for `GET /api/filing/anchor` signatures.
...
1. **Clone and configure:**
   - Copy `.env.example` to `.env` at repo root and set secrets (kept out of git).
