...
app.post("/api/filing/chat", async (req, res) => {
  ...
  const authMode = (process.env.AUTH_DECISION_MODE || "permissive").toLowerCase();
  ...
  switch (authMode) {
    case "enforce":
      // Strict mode: Use OpenAI's decision as-is
      decision = inferred.used ? inferred.decision : "ALLOW";
      summary = inferred.used
        ? inferred.summary
        : "Request allowed (LLM not configured)";
      break;

    case "audit":
      // Audit mode: Log DENY but always allow execution
      decision = "ALLOW";
      summary = inferred.used ? inferred.summary : "Request allowed (audit mode)";
    ...
    case "permissive":
    default:
      // Permissive mode (default): Always allow, OpenAI only provides conversational summary
      decision = "ALLOW";
      summary = inferred.used
        ? inferred.summary
  ...
  const authority = await appendLedger({
    ...
    payload: {
      ...
      summary,
      ...(authMode !== "permissive" && { authMode }), // Only include if not default
      ...(authNotes && { authNotes }), // Only include if present
      inputRef: intent.id,
      llm: inferred.used
        ? {
            used: true,
            model: inferred.model,
            // Include rawDecision only if it differs from final decision (for audit trail)
  ...
  const reply = inferred.used
    ? inferred.summary
    : "Got it. I've logged your intent and it's been allowed (LLM not configured for conversational responses).";

  res.status(200).json({ ok: true, reply, sessionId, events: [intent, authority] });
});
