...
export class SessionCompletionRuntime {
  ...
  private async routeToDestination(
    ...
    try {
      ...
      if (!retrying && this.config.retryAttempts > 0) {
        await this.retryWithBackoff(events, destination);
      }
      // If already retrying, do not call retryWithBackoff again (prevents recursion)
    }
  }
  ...
  private async retryWithBackoff(
    ...
    for (let attempt = 1; attempt <= this.config.retryAttempts; attempt++) {
      ...
      try {
        // Call the low-level send methods directly to avoid recursion
        switch (destType) {
          case "database":
            await this.sendToDatabase(events, destination);
            break;
